cpg.assoc_mod <- function (beta.val, indep, covariates = NULL, data = NULL, logit.transform = FALSE, 
          chip.id = NULL, subset = NULL, random = FALSE, fdr.cutoff = 0.05, 
          large.data = TRUE, fdr.method = "BH", logitperm = FALSE) 
{
  p.name.holder <- list(deparse(substitute(beta.val)), deparse(substitute(chip.id)), 
                        cpg.everything(deparse(substitute(indep))))
  beta.val <- as.matrix(beta.val)
  gc()
  if (is.null(ncol(beta.val))) {
    beta.val <- as.matrix(beta.val)
  }
  if (ncol(beta.val) == 1) 
    beta.val <- t(beta.val)
  ...
  if (class(covariates) == "formula") {
    variables <- gsub("[[:blank:]]", "", strsplit(as.character(covariates)[2], 
                                                  "+", fixed = TRUE)[[1]])
  ...
  gc()
  if (!large.data) {
    results <- cpg.work(beta.val, indep, covariates, data, 
                        logit.transform, chip.id, subset, random, fdr.cutoff, 
                        callarge = FALSE, fdr.method, logitperm)
  }
  else {
    ...
    if (.Platform$OS.type == "windows") {
      # maxmemo <- memory.limit()/15 #UPDATED source code for cpg.assoc() function since memory.limit() is no longer supported (R version > 4.0.0).
      maxmemo <- 50000/15 
    }
    else {
      ...
      if (whatsystem %in% c("Linux", "Solaris")) {
        ...
        maxmemo <- as.numeric(strsplit(info[2], "\\s+")[[1]][4])/(1024 * 
                                                                    15)
      }
      else {
        top_output <- system("top -l1 -n 20 | grep -Ei \"mem|vm\"", 
                             intern = TRUE)
        there <- gsub("[[:alpha:]]", "", top_output[2])
        mborgb <- gsub("[[:digit:]]", "", top_output[2])
        mborgb <- gsub("free", "", mborgb)
        locat <- gregexpr("[[:alpha:]]", mborgb)[[1]]
        mbgb <- substr(mborgb, locat[length(locat)], 
                       locat[length(locat)])
        positi <- gregexpr("[[:blank:]]", there)[[1]]
        maxmemo <- as.numeric(substr(there, (positi[length(positi) - 
                                                      1] + 1), (positi[length(positi)] - 1)))/15
        if (toupper(mbgb) == "G") {
          maxmemo = maxmemo * 1024
        }
      }
    }
    mainobjectsize <- object.size(beta.val)/1048576
    if (!is.matrix(beta.val)) {
      beta.val <- as.matrix(beta.val)
    }
    allresults <- list()
    gc()
    ...
    if (logit.transform) {
      ...
      if (length(onevalues) > 0 | length(zerovalues) > 
          0) {
    ...
    for (j in 1:i) {
      gc()
      if (j < i) {
        allresults[[j]] <- cpg.work(beta.val[1:div, ], 
                                    indep, covariates, data, logit.transform, chip.id, 
                                    subset, random, fdr.cutoff, callarge = TRUE, 
        ...
        allresults[[j]] <- cpg.work(beta.val, indep, 
                                    covariates, data, logit.transform, chip.id, 
                                    subset, random, fdr.cutoff, callarge = TRUE, 
                                    fdr.method, logitperm, big.split = big.split)
        gc()
      }
